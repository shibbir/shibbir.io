...
[Gulp](https://gulpjs.com/) is a cross-paltform task runner for web development. When working on a javascript projects, quite often we have to do some repetitive tasks such as minifying the css or javascript,
concatenating them into one file to reduce the HTTP round trip between server to the client browser, optimizing images, running the tests etc. Depending on the size of the projects, managing those tasks can be very time
consuming and tedious. But with the help of a task runner you can really automate this whole process and just focus on the application.
...
`src(globs, [options])`: Accepts a globs which is a string array and returns a stream that produces Vinyl objects. Vinyl is a metadata object that describes a file.
...
Create a **_gulpfile.js_** or **_Gulpfile.js_** file in root directory that automatically loads when you run the `gulp` command. The syntax for gulp is `gulp <task> <othertask>`. Any exported functions from **_gulpfile.js_** will be registered
...
Gulp plugins are small utility library that encapsulate common functionality to transform files in a pipeline. You can use multiple plugins in a single task. Plugins are a great way to reuse functionality from different
places in Gulpfile. Currently there are 3807 plugins available and the list is still growing. You can use plugins to do things like css minification, concatenating javascripts, running server etc. Lets look at some
...
First we brought in the required libraries. Then, we created a task called `style`. Inside that task we told gulp what is the file path for our scss file using `src` api. Than we compile it down to css using the `sass()` method that
...
Easy, right? Although we've gone through only the basics of Gulp. Gulp can do much more than this and can be very complex or very simple depending on the project. Here are some important ponits to remember:
...
* If your gulpfile become too big, you can split each task into its own file. More on [here](https://gulpjs.com/docs/en/getting-started/javascript-and-gulpfiles#splitting-a-gulpfile).
